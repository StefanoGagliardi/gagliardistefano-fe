 ...
 * Script start
 */
interface DevelopmentProcessInterface {
  introColumnContent?: JSX.Element;
}
const DevelopmentProcess: FC<DevelopmentProcessInterface> = (
  props: DevelopmentProcessInterface
): ReactElement => {
  const { introColumnContent } = props;

  // first version onClick
  ...
  // Deprecated onClick on svg path, now it's aniamted via scroll
  ...
  // Set ScrollTrigger and page scroll for SVG Path animation
  useIsomorphicLayoutEffect(() => {
    ...
    const tl = gsap.timeline({
      scrollTrigger: {
        ...
        end: () => '+=2500', // '+=' + refElements.get(0).offsetWidth,
        scrub: true,
        pin: true,
        markers: false,
        // anticipatePin: 1,
    ...
    tl.fromTo(
      ...
      },
      {
        ...
        duration: 0.2,
        onStart: () => {
          console.log('=== onStart - CIRCLE_V2-2');
          setActiveCircle('CIRCLE_V2-2');
        },
        onReverseComplete: () => {
          console.log('=== onReverseComplete - CIRCLE_V2-2');
          setActiveCircle('CIRCLE_V2');
        },
      }
    )
      .add('scene-2')
      .fromTo(
        ...
        },
        {
          ...
          onStart: () => {
            ...
            setActiveCircle('CIRCLE_V2-3');
          },
          onReverseComplete: () => {
            console.log('=== onReverseComplete - CIRCLE_V2-3');
            setActiveCircle('CIRCLE_V2-2');
          },
        }
      )
      ...
      .fromTo(
        ...
        },
        {
          ...
          onStart: () => {
            ...
            setActiveCircle('CIRCLE_V2-4');
          },
          onReverseComplete: () => {
            console.log('=== onReverseComplete - CIRCLE_V2-4');
            setActiveCircle('CIRCLE_V2-3');
          },
        }
      )
      ...
      .fromTo(
        ...
        },
        {
          ...
          onStart: () => {
            console.log('On start THIRD CIRCLE');
            setActiveCircle('CIRCLE_V2-5');
          },
          onReverseComplete: () => {
            console.log('=== onReverseComplete - CIRCLE_V2-5');
            setActiveCircle('CIRCLE_V2-4');
          },
        }
      )
      ...
      .fromTo(
        ...
        },
        {
          ...
          onStart: () => {
            console.log('On start THIRD CIRCLE');
            setActiveCircle('CIRCLE_V2-6');
          },
          onReverseComplete: () => {
            console.log('=== onReverseComplete - CIRCLE_V2-6');
            setActiveCircle('CIRCLE_V2-5');
          },
        }
      );
  ...
  // When Svg section change, set entry effect on text box on left column
  useIsomorphicLayoutEffect(() => {
    const refElements: Map<number, HTMLElement> = gsapRef.current; // Array of dom references
    if (activeCircle === 'CIRCLE_V2') {
      gsap.fromTo(
        refElements.get(7),
        { y: -100, autoAlpha: 0 },
        { y: 0, duration: 1, autoAlpha: 1 }
      );
    }
    if (activeCircle === 'CIRCLE_V2-2') {
      gsap.fromTo(
        refElements.get(8),
        { y: -100, autoAlpha: 0 },
        { y: 0, duration: 1, autoAlpha: 1 }
  ...
  return (
    <div className="container mx-auto" ref={(ref: any) => setGsapRef(ref, 0)}>
      {typeof introColumnContent !== 'undefined' && (
      ...
      <div className="grid grid-cols-12 gap-1 mt-60px">
        <div className="col-start-3 col-span-4 self-center">
          ...
          {activeCircle === 'CIRCLE_V2-2' && (
            <div
              className="px-20px py-20px bg-white relative z-3 max-w-[90%]"
              ref={(ref: any) => setGsapRef(ref, 8)}
            >
              <h3 className={cn('text-accent', 'text-lg', 'font-bold', 'mb-2')}>
          ...
          {activeCircle === 'CIRCLE_V2-3' && (
            <div
              className="px-20px py-20px bg-white relative z-3 max-w-[90%]"
              ref={(ref: any) => setGsapRef(ref, 9)}
            >
              <h3 className={cn('text-accent', 'text-lg', 'font-bold', 'mb-2')}>
              ...
              <p className={cn('text-paragraph')}>
                Se approvi…
                <strong> costruiamo un prototipo di e-commerce</strong>{' '}
                personalizzato in base alle tue esigenze/preferenze, sia a
                livello estetico che funzionale.
              </p>
            </div>
          )}

          {activeCircle === 'CIRCLE_V2-4' && (
            <div
              className="px-20px py-20px bg-white relative z-3 max-w-[90%]"
              ref={(ref: any) => setGsapRef(ref, 10)}
            >
              <h3 className={cn('text-accent', 'text-lg', 'font-bold', 'mb-2')}>
                Fase 4 - Messa Online
              </h3>
          ...
          {activeCircle === 'CIRCLE_V2-5' && (
            <div
              className="px-20px py-20px bg-white relative z-3 max-w-[90%]"
              ref={(ref: any) => setGsapRef(ref, 11)}
            >
              <h3 className={cn('text-accent', 'text-lg', 'font-bold', 'mb-2')}>
                Fase 5 - Promozione
              </h3>
              <p className={cn('text-paragraph')}>
                ...
                <strong>strategia di marketing giusta</strong> per intercettare
                un gran numero di clienti in target, rendendo attraente ai loro
                occhi il tuo e-Commerce nuovo fiammante.
              </p>
            </div>
          )}
          {activeCircle === 'CIRCLE_V2-6' && (
            <div
              className="px-20px py-20px bg-white relative z-3 max-w-[90%]"
              ref={(ref: any) => setGsapRef(ref, 12)}
            >
              <h3 className={cn('text-accent', 'text-lg', 'font-bold', 'mb-2')}>
                ...
                <g id="CIRCLES_WRAPPER" transform="translate(772.054 169.137)">
                  ...
                  <g id="FASE-5">
                    <path
                      ...
                      strokeWidth="45"
                      strokeDasharray={
                        activePath === 'CIRCLE_V2-5' ? '1000 0' : '1000 790'
                      }
                      className={cn({
